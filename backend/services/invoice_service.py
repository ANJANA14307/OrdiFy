...
def build_invoice_pdf(
    ...
    if customer:
        ...
        if customer.get("instagram_username"):
            customer_contact_parts.append(
                f"Instagram: @{customer.get('instagram_username')}"
            )
    ...
    story.append(Paragraph("Inventory • Orders • Customers • Payments", muted_style))
    ...
    story.append(
        Paragraph(
            ...
            f"{customer_contact if customer_contact else 'Customer details not available'}",
            normal_style,
        )
    )
    story.append(Spacer(1, 14))
...
def upload_invoice_to_supabase_storage(pdf_path: str, public_id: str):
...
def generate_invoice_for_payment(payment_id: str, user_id: str):
    ...
    public_id = f"invoice_{safe_order_number}_{payment_id}"
    ...
    pdf_url = upload_invoice_to_supabase_storage(
        pdf_path=pdf_path,
        public_id=public_id,
    )
    ...
    create_notification(
        ...
        message=f"Invoice generated successfully for order {order_number}.",
